...
class Webservices(object):
    ...
    def getPropTable(self):
        ...
        for p in props:
            ...
            if p.xpath('./td[1]/text()') != ['Prop299'] and p.xpath('./td[1]/text()') != ['Prop300'] and p.xpath('./td[1]/text()') != []:
        ...
        for p in propDict:
            if len(p) == 4:
                ...
                if int(response['recordCount']) > 1:
                    ...
                    p.append(plist)
                elif int(response['recordCount']) == 0:
                    continue
                else:
    ...
    def postReq(self, payloadData):
        ...
        payload = {'params': payloadData}
        print payloadData
        response = requests.post(url, data=payload, auth=(self.client, self.password))
        print response
